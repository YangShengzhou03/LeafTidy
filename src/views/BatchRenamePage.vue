  ...
  <div class="page-container">
    ...
    <div class="page-content">
      ...
      <div class="action-bar">
        <el-button type="primary" :disabled="templateParts.length === 0 || workDirs.length === 0" :loading="renaming" @click="startRename">
          开始重命名
        </el-button>
      </div>
        ...
        <div class="result-section" v-if="successResults.length > 0">
          ...
          <div class="result-list">
            <div v-for="(result, index) in successResults" :key="'success-' + index" class="result-item success">
              <el-icon><SuccessFilled /></el-icon>
              <span class="result-name clickable" @click="openFile(result.source_path)" :title="result.source_path">{{ getFileName(result.source_path) }}</span>
              <span class="result-arrow">→</span>
              <span class="result-target clickable" @click="openFile(result.target_path)" :title="result.target_path">{{ result.new_name }}</span>
            </div>
          </div>
        </div>
      </div>
...
import type { WorkDirectory, RenameResult, RenameRule } from '@/types'
...
async function startRename() {
  ...
  try {
    ...
    const res = await invoke<RenameResult[]>('batch_rename', { 
      paths, 
    ...
    if (res.length > 0) {
      ...
      if (fail === 0) {
        ElMessage.success(`重命名完成，共处理 ${success} 个文件，已复制到输出目录`)
      } else {
        ElMessage.warning(`重命名完成，成功 ${success} 个，失败 ${fail} 个`)
      }
    } else {
      ElMessage.info('没有找到需要重命名的文件')
    }
  } catch (e: any) {
