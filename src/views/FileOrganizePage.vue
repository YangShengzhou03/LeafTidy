  ...
  <div class="page-container">
    ...
    <div class="page-content">
          ...
          <div class="path-builder">
            <div class="path-segments">
              ...
              >
                ...
                <el-icon class="segment-remove"><Close /></el-icon>
              </div>
            </div>
      ...
      <div class="action-bar">
        <el-button type="primary" @click="startOrganize" :disabled="!canOrganize" :loading="organizing">
          开始整理
        </el-button>
      </div>
        ...
        <div class="result-section" v-if="successResults.length > 0">
          ...
          <div class="result-list">
            <div v-for="(result, index) in successResults" :key="'success-' + index" class="result-item success">
              ...
              <span class="result-target clickable" @click="openFile(result.target_path)" :title="result.target_path">{{ getRelativePath(result.target_path, outputDir) }}</span>
            </div>
          </div>
        </div>
      </div>
...
import type { WorkDirectory, OrganizeResult } from '@/types'
...
const { logOrganizeResults } = useLog()
...
async function startOrganize() {
  if (!canOrganize.value) {
    ElMessage.warning('请先选择待处理目录、输出目录和分类标签')
    return
  }
  ...
  try {
    ...
    if (res.length > 0) {
      ...
      if (fail === 0) {
        ElMessage.success(`整理完成，共处理 ${success} 个文件`)
      } else {
        ElMessage.warning(`整理完成，成功 ${success} 个，失败 ${fail} 个`)
      }
    } else {
      ElMessage.info('没有找到需要整理的文件')
    }
  } catch (e: any) {
    ElMessage.error(`整理失败: ${e}`)
  } finally {
