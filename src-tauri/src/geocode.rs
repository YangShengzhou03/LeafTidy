...
impl AdminCodeMapping {
    ...
    fn add_from_line(&mut self, line: &str) {
        ...
        else if feature_class == "A" && feature_code == "ADM2" && !admin1.is_empty() && !admin2.is_empty() {
            let key = format!("{admin1}_{admin2}");
...
fn extract_chinese_name(all_names: &str) -> String {
    let chinese_names: Vec<String> = all_names
        .split(',')
        .filter(|name| name.chars().any(|c| ('\u{4e00}'..='\u{9fff}').contains(&c)))
...
impl GeoName {
    ...
    fn axis_squared_distance(&self, other: &Self, axis: usize) -> f64 {
...
impl KDTree {
    ...
    fn build_tree(items: &mut [GeoName], depth: usize) -> Box<KDNode> {
        assert!(!items.is_empty(), "build_tree called with empty items");
    ...
    fn find_nearest_node<'a>(node: &'a KDNode, search: &GeoName, depth: usize) -> &'a KDNode {
        ...
        let mut best = next
            .as_ref()
            .map_or(node, |next_node| Self::find_nearest_node(next_node, search, depth + 1));
...
impl ReverseGeoCode {
    ...
    pub fn reverse_geocode(&self, latitude: f64, longitude: f64) -> super::models::GpsLocation {
        ...
        let (province, city, district) = self
            .kd_tree_admin
            .find_nearest(&search)
            .map_or_else(
                || {
        ...
        let place = self.kd_tree_places.find_nearest(&search).and_then(|place_unit| {
...
pub fn reverse_geocode(lat: f64, lng: f64) -> Result<super::models::GpsLocation, String> {
    get_geocoder().map_or_else(
        || Err("地理编码器未初始化，请确保 geodata/CN.txt 文件存在".to_string()),
        |geocoder| Ok(geocoder.reverse_geocode(lat, lng)),
    )
}