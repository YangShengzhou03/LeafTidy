...
use crate::cancel;
use crate::models::{ExifInfo, GpsLocation, ImageProcessResult, TaskProgress};
    ...
    for (i, path_str) in paths.iter().enumerate() {
        progress_callback(&TaskProgress {
            ...
            percentage: {
                ...
                {
                    if total > 0 { i as f64 / total as f64 * 100.0 } else { 100.0 }
                }
            },
        });
...
// ponytail: 5 个 bool 是前端传入的独立功能开关，结构合理，无需重构
