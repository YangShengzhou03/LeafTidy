...
use crate::metadata;
use crate::models::{
    AppSettings, BatchOperationResult, CancelResult, CleanupResult, DirectoryStats, DuplicateScanResult,
    FileEntry, GpsLocation, ImageProcessResult, LogEntry, OrganizeProgress, OrganizeResult, OrganizeRule,
    RenameResult, RenameRule, TaskProgress,
};
...
// tauri::command 的参数按值接收是框架反序列化要求，不适用 needless_pass_by_value
...
pub fn clean_duplicates(
    ...
    Ok(file_ops::clean_duplicates(paths, keep_original))
}
