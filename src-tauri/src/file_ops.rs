...
use crate::metadata;
use crate::models::{
    BatchOperationResult, CleanupFile, CleanupResult, DirectoryStats, DuplicateFile, DuplicateGroup,
    DuplicateScanResult, FileEntry, GpsLocation, OperationResult, OrganizeMetadata, OrganizeProgress,
    OrganizeResult, OrganizeRule, RenameResult, RenameRule, TaskProgress,
};
...
fn get_gps_location(file_path: &Path) -> Option<GpsLocation> {
    let path_str = file_path.to_string_lossy().to_string();
...
fn get_province(file_path: &Path) -> String {
    get_gps_location(file_path)
        .and_then(|location| location.province)
        .unwrap_or_default()
}

fn get_city(file_path: &Path) -> String {
    get_gps_location(file_path)
        .and_then(|location| location.city)
        .unwrap_or_default()
}

fn get_district(file_path: &Path) -> String {
    get_gps_location(file_path)
        .and_then(|location| location.district)
        .unwrap_or_default()
}

fn get_place(file_path: &Path) -> String {
    get_gps_location(file_path)
        .and_then(|location| location.place)
        .unwrap_or_default()
}

fn get_camera_make(file_path: &Path) -> String {
    let path_str = file_path.to_string_lossy().to_string();
    if !metadata::is_image_file(&path_str) {
        return String::new();
    }
    ...
    metadata::read_exif(&path_str)
        .map(|exif| exif.camera_make.unwrap_or_default())
        .unwrap_or_default()
}

fn get_camera_model(file_path: &Path) -> String {
    let path_str = file_path.to_string_lossy().to_string();
    if !metadata::is_image_file(&path_str) {
        return String::new();
    }
    ...
    metadata::read_exif(&path_str)
        .map(|exif| exif.camera_model.unwrap_or_default())
        .unwrap_or_default()
}
...
fn get_tag_value(
    ...
    match tag {
        ...
        "province" => {
            if is_media_file {
                let province = get_province(file_path);
                if province.is_empty() {
                    "未知省份".to_string()
                } else {
                    province
        ...
        "district" => {
            if is_media_file {
                let district = get_district(file_path);
                if district.is_empty() {
                    "未知区县".to_string()
                } else {
                    district
        ...
        "make" => {
            if is_media_file {
                let make = get_camera_make(file_path);
                if make.is_empty() {
                    "未知品牌".to_string()
                } else {
                    make
        ...
        "model" => {
            if is_media_file {
                let model = get_camera_model(file_path);
                if model.is_empty() {
                    "未知型号".to_string()
                } else {
                    model
...
// ponytail: 保持原实现的大小写敏感扩展名比较（行为不变，如需大小写不敏感再统一改）
...
pub fn cleanup_auxiliary_files_with_progress<F>(
    files: &[String],
    mut progress_callback: F,
) -> Result<BatchOperationResult, String>
where
